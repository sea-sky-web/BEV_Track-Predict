...
def test_view_coherent_hflip_flips_images_bev_and_aux_labels():
    ...
    assert torch.allclose(out_imgs, torch.flip(imgs, dims=(-1,)), atol=1e-6)
    assert torch.allclose(out_map, torch.flip(map_gt, dims=(-1,)), atol=1e-6)
    assert torch.allclose(out_aux, torch.flip(aux_gt, dims=(-1,)), atol=1e-6)
