...
def warp_perspective_torch(
    ...
    device = src.device
    dtype = src.dtype
    # Performance optimization: Use float32 coordinate mapping. This has a negligible
    # effect on precision while significantly accelerating mapping speed.
    ...
    # Performance optimization: Use torch.bmm for batched matrix multiplication.
    # Noticeably faster than regular @ operators with nested batched
    # operations.
    ...
    # Performance optimization: Directly map mathematically invalid mappings (z <= 1e-6)
    # to out of bounds coordinates by leveraging grid_sample native padding.
    # This avoids slower clamp, isfinite checks, and slow division masking.
    invalid = z <= 1e-6
    z_safe = torch.where(
        invalid,
        torch.tensor(
