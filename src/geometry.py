...
def warp_perspective_torch(
    ...
    device = src.device
    dtype = src.dtype
    # Performance optimization: Use float32 instead of float64 for coordinate
    # arithmetic
    ...
    M_inv = torch.linalg.inv(
        M_src2dst.to(
            dtype=calc_dtype,
            device=device))  # (B, 3, 3)
    # Performance optimization: Use torch.bmm which is highly performant
    src_h = torch.bmm(M_inv, dst_h)  # (B, 3, Hd*Wd)

    # 从齐次坐标恢复笛卡尔坐标
    z = src_h[:, 2]
    # Performance optimization: identify invalid points and assign 1.0 to
    # z_safe to prevent division errors.
    invalid = z <= 1e-6
    ...
    # Performance optimization: Push invalid coordinates out-of-bounds rather than using complex
    # arithmetic penalties or slow mask-filling operations like
    # torch.nan_to_num.
...
def create_grid_sampler(
    ...
    def sampler(src: torch.Tensor) -> torch.Tensor:
        ...
        # Performance optimization: Use torch.bmm for better performance
        src_h = torch.bmm(M_inv, dst_h.to(dtype=dtype).expand(B, -1, -1))

        z = src_h[:, 2]
        # Performance optimization: push invalid coordinates out of bounds
        # using padding_mode instead of clamp_min
        invalid = z <= 1e-6
        z_safe = torch.where(invalid, torch.ones_like(z), z)
