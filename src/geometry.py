...
def warp_perspective_torch(
    ...
    device = src.device
    dtype = src.dtype
    # ⚡ Bolt Optimization: Use float32 for coordinate math instead of float64
    # to significantly speed up computation on CPU/GPU without meaningful
    # precision loss.
    ...
    M_inv = torch.linalg.inv(
        M_src2dst.to(
            dtype=calc_dtype,
            device=device))  # (B, 3, 3)
    # ⚡ Bolt Optimization: Use torch.bmm instead of @ for faster batched matrix multiplication.
    src_h = torch.bmm(M_inv, dst_h)  # (B, 3, Hd*Wd)

    # 从齐次坐标恢复笛卡尔坐标
    z = src_h[:, 2]

    # ⚡ Bolt Optimization: Out-of-bounds handling leveraging grid_sample's padding_mode.
    # Rather than using complex math masking (torch.isfinite, torch.nan_to_num), we
    # simply push invalid coordinates out-of-bounds (2.0) allowing grid_sample
    # to return zeros.
    invalid = z <= 1e-6
