...
def warp_perspective_torch(
    ...
    # Optimisation: float32 is sufficient for perspective geometry math
    # on BEV ranges and runs >30% faster than float64
    ...
    # Optimisation: Use batched matrix multiplication directly
    src_h = torch.bmm(M_inv, dst_h)  # (B, 3, Hd*Wd)

    # 从齐次坐标恢复笛卡尔坐标
    z = src_h[:, 2]

    # Fast invalidation of out-of-bounds geometries
    invalid = z <= 1e-6

    # Avoid division by zero naturally
    z = torch.where(invalid, 1.0, z)
    ...
    # Optimisation: naturally push invalid points out of grid sample boundaries
    # and rely on F.grid_sample's internal padding_mode='zeros' rather than
    # constructing complex nan-to-num masks over output arrays
...
def create_grid_sampler(
    ...
    def sampler(src: torch.Tensor) -> torch.Tensor:
        ...
        # Optimisation: Use bmm instead of @
