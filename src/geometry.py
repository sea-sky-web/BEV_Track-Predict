...
def build_mvdet_proj_mat(
    ...
    Example:
        >>> from calibration import read_intrinsics, read_opencv_xml_vec, scale_intrinsics
...
def warp_perspective_torch(
    ...
    device = src.device
    dtype = src.dtype
    # Optimization: Use float32 for faster coordinate arithmetic
    ...
    M_inv = torch.linalg.inv(
        M_src2dst.to(
            dtype=calc_dtype,
            device=device))  # (B, 3, 3)
    # Optimization: Use bmm instead of matmul for faster batched matrix
    # multiplication
    src_h = torch.bmm(M_inv, dst_h)  # (B, 3, Hd*Wd)

    # 从齐次坐标恢复笛卡尔坐标
    z = src_h[:, 2]
    # Handle out-of-bounds natively by pushing invalid coordinates
    # out-of-bounds (e.g. 2.0)
    invalid = z <= 1e-6
    # Avoid div by zero by clamping z
    ...
    # Push invalid coordinates out of bounds leveraging grid_sample's
    # padding_mode
