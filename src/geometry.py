...
def warp_perspective_torch(
    ...
    # ⚡ Bolt Optimization: Use float32 instead of float64 for faster coordinate math
    ...
    M_inv = torch.linalg.inv(
        M_src2dst.to(
            dtype=calc_dtype,
            device=device))  # (B, 3, 3)
    # ⚡ Bolt Optimization: Use batched matrix multiplication (bmm) instead of matmul
    src_h = torch.bmm(M_inv, dst_h)  # (B, 3, Hd*Wd)

    # 从齐次坐标恢复笛卡尔坐标
    z = src_h[:, 2]

    # ⚡ Bolt Optimization: Simplify out-of-bounds handling.
    # Instead of computing complex valid masks and using slow torch.nan_to_num,
    # we identify invalid points (z <= 1e-6), avoid div-by-zero, and push them
    # out of bounds to let grid_sample handle padding efficiently.
    invalid = z <= 1e-6
    z_safe = z.clone()
    z_safe[invalid] = 1.0  # 防止除以 0
    ...
    # ⚡ Bolt Optimization: Push invalid points out of grid bounds (2.0)
    x_norm[invalid] = 2.0
    y_norm[invalid] = 2.0
...
def create_grid_sampler(
    ...
    def sampler(src: torch.Tensor) -> torch.Tensor:
        ...
        # ⚡ Bolt Optimization: Use float32 for coordinate projection arithmetic
        calc_dtype = torch.float32

        M = proj_mat.to(dtype=calc_dtype, device=device)
        if M.dim() == 2:
            M = M.unsqueeze(0).expand(B, -1, -1)

        M_inv = torch.inverse(M)
        # ⚡ Bolt Optimization: Use batched matrix multiplication (bmm)
        src_h = torch.bmm(M_inv, dst_h.to(dtype=calc_dtype).expand(B, -1, -1))

        z = src_h[:, 2]

        # ⚡ Bolt Optimization: Better out of bounds and div-by-zero handling
        invalid = z <= 1e-6
        z_safe = z.clone()
        z_safe[invalid] = 1.0
        ...
        # Push invalid points out of grid bounds
        x_norm[invalid] = 2.0
        y_norm[invalid] = 2.0
