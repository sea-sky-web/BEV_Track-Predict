...
def build_mvdet_proj_mat(
    ...
    Example:
        ...
        >>> K_feat = K * 0.25  # Simplified scaling for doctest
...
def compute_bev_geometry_metadata(
    ...
    """Compute static per-view geometry metadata for each BEV cell.

    Uses the same inverse-homography math as warp_perspective_torch to
    determine, for each view and BEV position, whether the projection is
    valid and how far from the image border it lands.

    Args:
        proj_mats: (V, 3, 3) homography matrices (image feat → BEV grid).
        src_hw: (Hf, Wf) source feature-plane spatial size.
        dst_hw: (Hb, Wb) destination BEV grid spatial size.

    Returns:
        (V, 3, Hb, Wb) float32 tensor with channels:
            0 - valid_mask:     1.0 if BEV cell maps to a valid image region
            1 - border_margin:  normalised distance to nearest image edge (0–1)
            2 - coverage_count: fraction of views with valid coverage (shared)
    """
    V = proj_mats.shape[0]
    Hs, Ws = src_hw
    Hd, Wd = dst_hw

    xs = torch.arange(Wd, dtype=torch.float64)
    ...
    for vi in range(V):
        M = proj_mats[vi].to(torch.float64)
        try:
        ...
        src_h = M_inv @ dst_h  # (3, Hd*Wd)
        z = src_h[2]
        z_safe = z.clone()
        z_safe[z_safe.abs() < 1e-6] = 1e-6
        x = src_h[0] / z_safe
        y = src_h[1] / z_safe
        ...
        margin[~valid] = 0.0
        bm = margin.reshape(Hd, Wd).to(torch.float32)

        per_view.append(torch.stack([vm, bm]))  # (2, Hd, Wd)
        valid_masks.append(vm)

    coverage = torch.stack(valid_masks).sum(dim=0) / V  # (Hd, Wd)
    coverage = coverage.unsqueeze(0)  # (1, Hd, Wd)

    result = []
    for pv in per_view:
        result.append(torch.cat([pv, coverage], dim=0))  # (3, Hd, Wd)

    return torch.stack(result)  # (V, 3, Hd, Wd)
...
def warp_perspective_torch(
    ...
    # 优化：使用 float32 坐标算术以获得更高性能
    # Calculate inverse in float64 for numerical stability,
    # then use float32 for coordinate grid arithmetic
    ...
    dst_h = dst_h.unsqueeze(0).expand(B, -1, -1)

    M_inv = torch.linalg.inv(M_src2dst.to(dtype=torch.float64, device=device)).to(calc_dtype)
    # 优化：使用 bmm 替代广播矩阵乘法
    src_h = torch.bmm(M_inv, dst_h)

    z = src_h[:, 2]
    # 优化：简单的越界掩码处理，避免复杂的 clamp
    invalid = z <= 1e-6
    z = torch.where(invalid, torch.ones_like(z), z)
    x = src_h[:, 0] / z
    y = src_h[:, 1] / z

    ws_denom = max(Ws - 1.0, 1.0)
    hs_denom = max(Hs - 1.0, 1.0)
    x_norm = 2.0 * (x / ws_denom) - 1.0
    y_norm = 2.0 * (y / hs_denom) - 1.0

    # 优化：利用 grid_sample padding_mode 原生处理将越界坐标直接推到 2.0
    invalid_fill = torch.full_like(x_norm, 2.0)
    x_norm = torch.where(invalid, invalid_fill, x_norm)
    y_norm = torch.where(invalid, invalid_fill, y_norm)

    if min_valid_ratio > 0.0:
        in_bounds = (~invalid) & (x >= 0.0) & (x <= float(Ws - 1)) & (y >= 0.0) & (y <= float(Hs - 1))
