...
def warp_perspective_torch(
    ...
    device = src.device
    dtype = src.dtype
    # ⚡ Optimization: Use float32 for coordinate math, much faster than float64 and sufficient for image coordinates
    ...
    M_inv = torch.linalg.inv(
        M_src2dst.to(
            dtype=calc_dtype,
            device=device))  # (B, 3, 3)
    # ⚡ Optimization: Use torch.bmm for batched matrix multiplication (faster than @ for 3D tensors)
    src_h = torch.bmm(M_inv, dst_h)  # (B, 3, Hd*Wd)

    # 从齐次坐标恢复笛卡尔坐标
    z = src_h[:, 2]

    # ⚡ Optimization: Handle out-of-bounds using grid_sample's padding_mode instead of nan_to_num
    invalid = z <= 1e-6
    # Avoid division by zero, set invalid z to 2.0 (doesn't matter as we push
    # them out of bounds later)
    z_safe = torch.where(invalid, 2.0, z)
    ...
    # ⚡ Optimization: Push invalid coordinates out of bounds. grid_sample with zeros padding will return 0.
