...
def warp_perspective_torch(
    ...
    # [Optimization] Use float32 and torch.bmm for ~30% faster execution
    M_inv = torch.linalg.inv(M_src2dst.to(torch.float32))  # (B, 3, 3)
    ...
    # [Optimization] Push invalid coordinates out-of-bounds rather than using complex arithmetic penalties
    invalid = z <= 1e-6
    ...
    x_norm = 2.0 * (x / max(Ws - 1, 1)) - 1.0
    y_norm = 2.0 * (y / max(Hs - 1, 1)) - 1.0

    # [Optimization] Leverage grid_sample's padding_mode by forcing invalid coords to 2.0 (out of bounds)
    x_norm = torch.where(invalid, 2.0, x_norm)
    y_norm = torch.where(invalid, 2.0, y_norm)

    # 构建采样网格：(B, Hd, Wd, 2)
    grid = torch.stack([x_norm, y_norm], dim=-
                       1).reshape(B, Hd, Wd, 2).to(dtype)
