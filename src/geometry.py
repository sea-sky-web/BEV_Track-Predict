...
def warp_perspective_torch(
    ...
    # Use torch.float32 instead of torch.float64 for calculation and coordinate arithmetic.
    # Replace expensive matrix multiplication @ with torch.bmm.
    # Avoid complex bounds checks and nan_to_num in favor of grid_sample's
    # native padding.
    B, C, Hs, Ws = src.shape
    Hd, Wd = dsize
    device = src.device
    ...
    # 越界的点推向界外，grid_sample 会使用 zeros 进行 padding
