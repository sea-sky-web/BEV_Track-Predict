...
def warp_perspective_torch(
    ...
    # ⚡ Bolt: Use float32 coordinate arithmetic and avoid explicit .clamp() / nan_to_num() operations
    # by pushing invalid coordinates out-of-bounds (e.g., to 2.0) which
    # leverages grid_sample's native padding_mode.
...
def create_grid_sampler(
    ...
    def sampler(src: torch.Tensor) -> torch.Tensor:
        ...
        # ⚡ Bolt: Optimized inverse and matrix mults in float32
        ...
        # ⚡ Bolt: Avoid division by zero inherently instead of explicit inf/nan handling
        z = src_h[:, 2]
        invalid = ~(z > 1e-6)
        z = torch.where(invalid, torch.ones_like(z), z)
        x = src_h[:, 0] / z
