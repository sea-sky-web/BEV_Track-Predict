...
def warp_perspective_torch(
    ...
    device = src.device
    dtype = src.dtype
    # ⚡ Bolt Optimization: Use float32 instead of float64 for faster computation and lower memory bandwidth.
    ...
    # ⚡ Bolt Optimization: Replace `M_inv @ dst_h` with `torch.bmm` for highly optimized batched matrix multiplication.
    src_h = torch.bmm(M_inv, dst_h)  # (B, 3, Hd*Wd)

    # 从齐次坐标恢复笛卡尔坐标
    z = src_h[:, 2]

    # ⚡ Bolt Optimization: Leverage grid_sample's native out-of-bounds handling (padding_mode='zeros').
    # Instead of slow nan_to_num or complex penalties, push invalid
    # coordinates out of bounds (e.g., 2.0).
    invalid = z <= 1e-6
    z_safe = torch.where(
        invalid,
        torch.tensor(
    ...
    # 越界的设置成2.0，利用 grid_sample 自带的 zeros padding 处理
    out_of_bounds_val = torch.tensor(2.0, dtype=calc_dtype, device=device)
    x_norm = torch.where(invalid, out_of_bounds_val, x_norm)
    y_norm = torch.where(invalid, out_of_bounds_val, y_norm)
