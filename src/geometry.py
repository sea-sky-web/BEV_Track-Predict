...
def warp_perspective_torch(
    ...
    device = src.device
    dtype = src.dtype
    # Use float32 for coordinate arithmetic to speed up compute
    ...
    # 从齐次坐标恢复笛卡尔坐标
    z = src_h[:, 2]
    # Leverage grid_sample's padding_mode by pushing invalid coordinates
    # out-of-bounds
    invalid = z <= 1e-6
    ...
    x_norm = x * (2.0 / ws_denom) - 1.0
    y_norm = y * (2.0 / hs_denom) - 1.0
