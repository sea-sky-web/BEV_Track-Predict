...
def warp_perspective_torch(
    ...
    # 🏎️ BOLT OPTIMIZATION: Use float32 coordinate arithmetic and torch.bmm
    # Memory guideline: float32 coordinate arithmetic with torch.bmm is highly
    # performant.
    ...
    # 🏎️ BOLT OPTIMIZATION: Use float32 meshgrid
    ...
    # 🏎️ BOLT OPTIMIZATION: Use torch.bmm for batched matrix multiplication
    M_inv = torch.linalg.inv(M_src2dst.to(dtype=calc_dtype, device=device))
    src_h = torch.bmm(M_inv, dst_h)

    z = src_h[:, 2]

    # 🏎️ BOLT OPTIMIZATION: Push invalid coordinates out of bounds natively
    # instead of using complex arithmetic penalties or slow nan_to_num mask filling.
    # We replace z <= 1e-6 with a safe value (1.0) just to avoid division by zero,
    # and later set the normalized coords to 2.0 (out of bounds for
    # grid_sample).
    invalid = z <= 1e-6
    z_safe = torch.where(invalid, 1.0, z)
    ...
    # 🏎️ BOLT OPTIMIZATION: push invalid points out of [-1, 1] bounds
    # so grid_sample padding_mode="zeros" handles them efficiently
    x_norm = torch.where(invalid, 2.0, x_norm)
    y_norm = torch.where(invalid, 2.0, y_norm)

    # 构建采样网格：(B, Hd, Wd, 2)
    grid = torch.stack([x_norm, y_norm], dim=-
                       1).reshape(B, Hd, Wd, 2).to(dtype=dtype)
...
def create_grid_sampler(
    ...
    def sampler(src: torch.Tensor) -> torch.Tensor:
        ...
        # 🏎️ BOLT OPTIMIZATION: Use torch.linalg.inv and torch.bmm for better performance
        M_inv = torch.linalg.inv(M)
        # Use bmm instead of @
        src_h = torch.bmm(M_inv, dst_h.to(dtype=dtype).expand(B, -1, -1))

        z = src_h[:, 2]
        # 🏎️ BOLT OPTIMIZATION: Push invalid coordinates out of bounds natively
        # avoiding clamp and using grid_sample's padding_mode
        invalid = z <= 1e-6
        z_safe = torch.where(invalid, 1.0, z)
        ...
        # push invalid points out of [-1, 1] bounds
        x_norm = torch.where(invalid, 2.0, x_norm)
        y_norm = torch.where(invalid, 2.0, y_norm)
