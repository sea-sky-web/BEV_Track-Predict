...
def warp_perspective_torch(
    ...
    - Uses float32 calculation for faster matrix math
    - Uses torch.bmm for batched matrix multiplication
    - Pushes invalid coordinates out of bounds natively rather than complex masking
...
def create_grid_sampler(
    ...
    # Pre-calculate normalized scaling factors
    w_scale = 2.0 / float(max(Ws - 1, 1))
    h_scale = 2.0 / float(max(Hs - 1, 1))

    def sampler(src: torch.Tensor) -> torch.Tensor:
        B = src.shape[0]
        dtype = src.dtype

        M = proj_mat.to(dtype=torch.float32, device=device)
        if M.dim() == 2:
            M = M.unsqueeze(0).expand(B, -1, -1)

        M_inv = torch.linalg.inv(M)
        src_h = torch.bmm(M_inv, dst_h.expand(B, -1, -1))
        ...
        x_norm = x * w_scale - 1.0
        y_norm = y * h_scale - 1.0
