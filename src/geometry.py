...
def warp_perspective_torch(
    ...
    # Optimization: Use float32 instead of float64 for faster coordinate
    # arithmetic
    ...
    M_inv = torch.linalg.inv(
        M_src2dst.to(
            dtype=calc_dtype,
            device=device))  # (B, 3, 3)
    # Optimization: Use torch.bmm instead of @ for better performance
    src_h = torch.bmm(M_inv, dst_h)  # (B, 3, Hd*Wd)

    # 从齐次坐标恢复笛卡尔坐标
    z = src_h[:, 2]
    # Optimization: Simplified validity check
    invalid = z <= 1e-6
    z_safe = torch.where(invalid, 1.0, z)  # Avoid division by zero
    ...
    # Optimization: Push invalid coordinates out-of-bounds instead of using
    # slow nan_to_num
...
def create_grid_sampler(
    ...
    def sampler(src: torch.Tensor) -> torch.Tensor:
        ...
        # Optimization: Use float32 for coordinate arithmetic
        calc_dtype = torch.float32
        ...
        # Optimization: Use torch.bmm instead of @
        src_h = torch.bmm(M_inv, dst_h.to(dtype=calc_dtype).expand(B, -1, -1))

        z = src_h[:, 2]
        # Optimization: Simplified out-of-bounds handling
        invalid = z <= 1e-6
        ...
        # Optimization: Push invalid coordinates out-of-bounds
        x_norm = torch.where(invalid, 2.0, x_norm)
        y_norm = torch.where(invalid, 2.0, y_norm)
