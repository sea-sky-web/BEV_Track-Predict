...
def warp_perspective_torch(
    ...
    device = src.device
    dtype = src.dtype
    # 性能优化: 使用 float32 代替 float64 进行坐标计算，速度更快且内存占用更小
    ...
    M_inv = torch.linalg.inv(
        M_src2dst.to(
            dtype=calc_dtype,
            device=device))  # (B, 3, 3)
    # 性能优化: 使用 torch.bmm 替代 matmul (@) 以加速批次矩阵乘法
    ...
    # 当 z <= 0 (即原始 z 未被截断前 <= 1e-6) 时，坐标无效，直接设置到远离 [-1, 1] 的地方
    # 使用 torch.where 比原先的整个 valid mask + nan_to_num 更高效，并且解决了 penalty
    # 为负时相互抵消的问题
    invalid = z <= 1e-6
    ...
    # 不再需要外层 nan_to_num 因为 grid_sample 自动用 0 填充了越界坐标
    return out
