...
def warp_perspective_torch(
    ...
    # 齐次坐标：(3, Hd*Wd)
    dst_h = torch.stack([xs, ys, torch.ones_like(xs)],
                        dim=0).reshape(3, -1)  # (3, Hd*Wd)
    dst_h = dst_h.unsqueeze(0).expand(B, -1, -1)  # (B, 3, Hd*Wd)

    # 逆向投影：src_h = inv(M) @ dst_h (float64 for inv precision, float32 for
    # matmul)
    M_inv = torch.linalg.inv(
        M_src2dst.to(
            dtype=torch.float64,
            device=device)).to(calc_dtype)  # (B, 3, 3)
    ...
    # 处理 Z <= 1e-6 的无效投影
    invalid = z <= 1e-6
    z = torch.where(invalid, 1.0, z)  # Avoid div by zero, discarded later
    ...
    # 越界点置于视场外，利用 grid_sample zeros padding 消除影响
