...
from calibration import scale_intrinsics  # noqa: F401
...
def warp_perspective_torch(
    ...
    # ⚡ Bolt Optimization:
    # 1. Use float32 instead of float64 for geometry arithmetic to leverage Tensor Cores and avoid memory overhead
    # 2. Use torch.bmm for batched matrix multiplication over generic matmul
    # 3. Replace complex masking and nan_to_num with simple coordinate pushing
    # to leverage grid_sample's padding_mode
    ...
    M_inv = torch.linalg.inv(
        M_src2dst.to(
            dtype=calc_dtype,
            device=device))  # (B, 3, 3)
    # Use torch.bmm for faster batched matrix multiplication
    src_h = torch.bmm(M_inv, dst_h)  # (B, 3, Hd*Wd)

    # 从齐次坐标恢复笛卡尔坐标
    z = src_h[:, 2]

    # ⚡ Bolt Optimization: Simplify division safely without complex masking
    # Push invalid coordinates out of bounds (z <= 0) to take advantage of
    # grid_sample's zero padding
    invalid = z <= 1e-6
    z_safe = torch.where(invalid, torch.ones_like(z), z)
    ...
    # Push invalid coordinates out of bounds to trigger padding_mode="zeros"
    ...
    if min_valid_ratio > 0.0:
        ...
        if valid_ratio < min_valid_ratio:
            raise ValueError(
                f"warp valid ratio {
                    valid_ratio:.6f} below required min_valid_ratio={
                    min_valid_ratio:.6f}")
...
def create_grid_sampler(
    ...
    def sampler(src: torch.Tensor) -> torch.Tensor:
        ...
        # ⚡ Bolt Optimization:
        # Use torch.bmm for batched matrix multiplication over generic matmul
        src_h = torch.bmm(M_inv, dst_h.to(dtype=dtype).expand(B, -1, -1))

        # ⚡ Bolt Optimization: Simply push invalid coordinates out of bounds to leverage grid_sample padding
        z = src_h[:, 2]
        invalid = z <= 1e-6
        z_safe = torch.where(invalid, torch.ones_like(z), z)
