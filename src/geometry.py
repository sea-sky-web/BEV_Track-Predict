...
def warp_perspective_torch(
    ...
    """
    # [BOLT OPTIMIZATION] - Use float32 coordinate arithmetic with torch.bmm
    # and handle out-of-bounds by pushing valid items using grid_sample's padding_mode
    # instead of complex arithmetic penalties and torch.nan_to_num.
    B, C, Hs, Ws = src.shape
    Hd, Wd = dsize
    device = src.device
    dtype = src.dtype
...
def create_grid_sampler(
    ...
    def sampler(src: torch.Tensor) -> torch.Tensor:
        ...
        # [BOLT OPTIMIZATION] - Use bmm and handle invalid items natively via grid_sample
