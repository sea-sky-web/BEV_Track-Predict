...
def warp_perspective_torch(
    ...
    device = src.device
    dtype = src.dtype
    # Optimized: use float32 for faster calculations
    ...
    M_inv = torch.linalg.inv(
        M_src2dst.to(
            dtype=calc_dtype,
            device=device))  # (B, 3, 3)
    # Optimized: use torch.bmm
    src_h = torch.bmm(M_inv, dst_h)  # (B, 3, Hd*Wd)

    # 从齐次坐标恢复笛卡尔坐标
    z = src_h[:, 2]
    # Optimized: simpler out-of-bounds handling instead of multiple clamped
    # variables
    invalid = z <= 1e-6
    z_safe = torch.where(
        invalid,
    ...
    # Optimized: leverage grid_sample's padding_mode by pushing invalid to 2.0
    # No need for nan_to_num and complex logic
    x_norm = torch.where(
        invalid,
        torch.tensor(
            2.0,
