...
def warp_perspective_torch(
    ...
    # ⚡ Bolt Optimization: Use float32 and bmm instead of float64 for geometric transformations
    # Push out-of-bounds rather than mask fill for massive speedups in
    ...
    # 逆向投影：src_h = inv(M) @ dst_h using bmm
    src_h = torch.bmm(
        M_inv, dst_h.unsqueeze(0).expand(
            B, -1, -1))  # (B, 3, Hd*Wd)
    ...
    # ⚡ Bolt Optimization: Push invalid coordinates out-of-bounds to use grid_sample's padding_mode="zeros"
