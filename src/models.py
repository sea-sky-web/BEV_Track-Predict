...
class MVDetLikeNet(nn.Module):
    ...
    def forward(
        ...
        # Batch all views together to avoid the loop over V
        x_views_flat = x_views.reshape(B * V, C, Hi, Wi)

        # Feature extraction
        f_flat = self.backbone(x_views_flat)

        # Interpolate to standard feature plane size
        ...
        # Single-view predictions (auxiliary task)
        imgs_logits_flat = self.img_head(f_flat)  # (B*V, 2, Hf, Wf)
        imgs_logits = imgs_logits_flat.view(B, V, 2, self.Hf, self.Wf)

        # Project to BEV
        # proj_mats is (V, 3, 3). Expand to (B, V, 3, 3) and then flatten to
        # (B*V, 3, 3)
        proj_mats_flat = self.proj_mats.unsqueeze(
            0).expand(B, V, 3, 3).reshape(B * V, 3, 3)
        bev_flat = warp_perspective_torch(
            f_flat, proj_mats_flat, dsize=(
                self.Hb, self.Wb))

        # Restore to (B, V, feat_ch, Hb, Wb) shape
        bev_stack = bev_flat.view(B, V, -1, self.Hb, self.Wb)

        if self.fusion_mode == "concat":
            # (B, V*feat_ch, Hb, Wb)
            bev_fused = bev_stack.reshape(B, -1, self.Hb, self.Wb)
        else:
