...
class ResNet50Stride8Trunk(nn.Module):
    ...
    def __init__(self, pretrained: bool = True, out_ch: int = 512):
        ...
        # 加载预训练权重
        weights = (
            torchvision.models.ResNet50_Weights.IMAGENET1K_V1 if pretrained else None
        )
...
class SpatialAwareConfidenceFusion(nn.Module):
    ...
    def forward(self, feats_bev: torch.Tensor) -> torch.Tensor:
        ...
        if feats_bev.ndim != 5:
            raise ValueError(
                f"Expected (B,V,C,H,W) BEV features, got {tuple(feats_bev.shape)}"
            )
...
class MVDetLikeNet(nn.Module):
    ...
    def __init__(
        ...
        if add_coord:
            ...
            # 创建坐标网格 [-1, 1]
            xs = (
                torch.linspace(-1, 1, self.Hb).view(self.Hb, 1).expand(self.Hb, self.Wb)
            )
            ys = (
                torch.linspace(-1, 1, self.Wb).view(1, self.Wb).expand(self.Hb, self.Wb)
            )
    ...
    def forward(self, x_views: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # [Bolt Optimization] Batched multi-view processing
        # Why: Iterating over views in a Python loop is slow and underutilizes the GPU.
        # Impact: By reshaping (B, V, ...) to (B*V, ...), we batch the feature extraction,
        #         interpolation, and perspective warping, yielding significantly faster execution.

        # Flatten batch and view dimensions
        x_flat = x_views.reshape(B * V, C, Hi, Wi)

        # 特征提取
        f_flat = self.backbone(x_flat)  # (B*V, feat_ch, Hi/8, Wi/8)
        _, feat_ch, _, _ = f_flat.shape

        # 插值到标准特征平面尺寸
        f_flat = F.interpolate(
            f_flat, size=(self.Hf, self.Wf), mode="bilinear", align_corners=False
        )

        # 单视角预测（辅助）
        imgs_logits_flat = self.img_head(f_flat)  # (B*V, 2, Hf, Wf)
        imgs_logits = imgs_logits_flat.reshape(B, V, 2, self.Hf, self.Wf)

        # 投影到 BEV
        # Expand proj_mats for batched warp
        M = self.proj_mats.unsqueeze(0).expand(B, V, 3, 3).reshape(B * V, 3, 3)
