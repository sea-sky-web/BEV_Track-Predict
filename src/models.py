...
class MVDetLikeNet(nn.Module):
    ...
    def forward(
        ...
        # 将批次和视角维度展平以进行并行处理
        flat_views = x_views.view(B * V, C, Hi, Wi)

        # 特征提取
        f = self.backbone(flat_views)  # (B*V, feat_ch, Hi/8, Wi/8)
        ...
        # 投影矩阵准备
        # proj_mats: (V, 3, 3) -> expand: (B, V, 3, 3) -> reshape: (B*V, 3, 3)
        M_flat = self.proj_mats.unsqueeze(0).expand(
            B, V, 3, 3).reshape(B * V, 3, 3)

        # 投影到 BEV
        bev = warp_perspective_torch(f, M_flat, dsize=(
            self.Hb, self.Wb))  # (B*V, feat_ch, Hb, Wb)

        # 恢复形状
        imgs_logits = img_logit.view(B, V, 2, self.Hf, self.Wf)
        bev_stack = bev.view(B, V, -1, self.Hb, self.Wb)

        if self.fusion_mode == "concat":
            bev_fused = bev_stack.flatten(1, 2)  # (B, V*feat_ch, Hb, Wb)
        else:
