...
class MVDetLikeNet(nn.Module):
    ...
    def forward(
        ...
        # ⚡ Bolt Optimization: Flatten multi-view dimension into batch dimension
        # to maximize GPU parallelization instead of using a Python for-loop
        C, Hi, Wi = x_views.shape[2:]
        x_flat = x_views.reshape(B * V, C, Hi, Wi)

        # 特征提取
        f = self.backbone(x_flat)  # (B*V, feat_ch, Hi/8, Wi/8)
        ...
        # 单视角预测（辅助）
        img_logit = self.img_head(f)  # (B*V, 2, Hf, Wf)

        # 恢复维度
        imgs_logits = img_logit.reshape(
            B, V, 2, self.Hf, self.Wf)  # (B, V, 2, Hf, Wf)

        # 投影到 BEV (Batched over B*V)
        M_flat = self.proj_mats.unsqueeze(0).expand(
            B, V, 3, 3).reshape(B * V, 3, 3)
        bev_flat = warp_perspective_torch(f, M_flat, dsize=(
            self.Hb, self.Wb))  # (B*V, feat_ch, Hb, Wb)

        # 恢复维度
        bev_stack = bev_flat.reshape(
            B, V, -1, self.Hb, self.Wb)  # (B, V, feat_ch, Hb, Wb)

        if self.fusion_mode == "concat":
            # 交换维度变为 (B, V*feat_ch, Hb, Wb) 并展开
