...
class MVDetLikeNet(nn.Module):
    ...
    def forward(
        ...
        # ⚡ Optimization: Vectorize view processing to maximize GPU parallelization.
        # Flatten batch and view dimensions to process all views simultaneously
        ...
        # Reshape features back to (B, V, ...) for projection
        f = f_flat.reshape(B, V, -1, self.Hf, self.Wf)

        feats_bev = []      # 投影后的特征
        # 逐视角投影到 BEV (since projection matrices differ per view)
        for vi in range(V):
            # 投影到 BEV
            M = self.proj_mats[vi].unsqueeze(0).expand(B, -1, -1)  # (B, 3, 3)
            bev = warp_perspective_torch(f[:, vi], M, dsize=(
                self.Hb, self.Wb))  # (B, feat_ch, Hb, Wb)
            feats_bev.append(bev)

        if self.fusion_mode == "concat":
            bev_fused = torch.cat(feats_bev, dim=1)  # (B, V*feat_ch, Hb, Wb)
        else:
            bev_stack = torch.stack(
                feats_bev, dim=1)  # (B, V, feat_ch, Hb, Wb)
