...
class MVDetLikeNet(nn.Module):
    ...
    def forward(
        ...
        # 🏎️ BOLT OPTIMIZATION: Vectorize feature extraction across batch and view dimensions
        # This replaces the Python for-loop and maximizes GPU utilization
        ...
        # Vectorize image head (auxiliary prediction)
        imgs_logits_flat = self.img_head(f_flat)  # (B*V, 2, Hf, Wf)
        imgs_logits = imgs_logits_flat.view(B, V, 2, self.Hf, self.Wf)

        # Vectorize BEV projection
        # M shape: (V, 3, 3) -> (1, V, 3, 3) -> (B, V, 3, 3) -> (B*V, 3, 3)
        M = self.proj_mats.unsqueeze(0).expand(
            B, -1, -1, -1).reshape(B * V, 3, 3)
        bev_flat = warp_perspective_torch(f_flat, M, dsize=(self.Hb, self.Wb))

        # Reshape back to (B, V, feat_ch, Hb, Wb)
        bev = bev_flat.view(B, V, -1, self.Hb, self.Wb)

        if self.fusion_mode == "concat":
            # Concat along channel dimension: (B, V*feat_ch, Hb, Wb)
            bev_fused = bev.view(B, V * bev.shape[2], self.Hb, self.Wb)
        else:
            bev_fused = self.confidence_fusion(bev)  # (B, feat_ch, Hb, Wb)
