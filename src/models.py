...
class MVDetLikeNet(nn.Module):
    ...
    def forward(
        ...
        # Flatten batch and view dimensions to parallelize operations across
        # all views
        ...
        # Interpolate to standard feature plane size (vectorized)
        ...
        # Single-view prediction (auxiliary) (vectorized)
        img_logits_flat = self.img_head(f_flat)  # (B*V, 2, Hf, Wf)
        imgs_logits = img_logits_flat.reshape(B, V, 2, self.Hf, self.Wf)

        # Projection to BEV (vectorized)
        # proj_mats is (V, 3, 3). Expand to (B, V, 3, 3) and reshape to (B*V,
        # 3, 3)
        ...
        # Prepare for fusion by reshaping back to separate batch and view
        bev_stack = bev_flat.reshape(B, V, -1, self.Hb, self.Wb)

        if self.fusion_mode == "concat":
            # Concatenate features along the channel dimension
            bev_fused = bev_stack.reshape(
                B, V * bev_stack.shape[2], self.Hb, self.Wb)
        else:
