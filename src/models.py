...
class MVDetLikeNet(nn.Module):
    ...
    def forward(
        ...
        Returns:
            tuple:
                ...
                - imgs_logits: 图像热图 logits (B, V, 2, Hf, Wf)
                              ...
                              - imgs_logits[:, :, 1]: 人脚
        """
        B, V, C_in, Hi, Wi = x_views.shape

        # Optimization: Vectorize the view dimension by flattening (B, V) into B*V
        # This allows parallel execution on the GPU rather than a sequential
        # Python for-loop
        x_views_flat = x_views.view(B * V, C_in, Hi, Wi)

        # 特征提取
        f_flat = self.backbone(x_views_flat)  # (B*V, feat_ch, Hi/8, Wi/8)
        ...
        # 投影到 BEV
        # Flatten proj_mats as well: proj_mats is (V, 3, 3)
        ...
        bev_stack = bev_flat.view(B, V, feat_ch, self.Hb, self.Wb)

        if self.fusion_mode == "concat":
            bev_fused = bev_stack.reshape(
                B, V * feat_ch, self.Hb, self.Wb)  # (B, V*feat_ch, Hb, Wb)
        else:
