...
class MVDetLikeNet(nn.Module):
    ...
    def forward(self, x_views: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # 单视角预测（辅助）
        img_logit_flat = self.img_head(f_flat)  # (B*V, 2, Hf, Wf)
        imgs_logits = img_logit_flat.reshape(B, V, 2, self.Hf, self.Wf)

        # 2. 向量化透视变换
        # self.proj_mats 是 (V, 3, 3) 静态 buffer
        # 扩展并重塑以匹配批次大小和视图数量，得到 (B*V, 3, 3)
        M_expanded = self.proj_mats.unsqueeze(0).expand(B, V, 3, 3).reshape(B * V, 3, 3)

        # 对所有视图和批次一起执行透视变换
        bev_flat = warp_perspective_torch(f_flat, M_expanded, dsize=(self.Hb, self.Wb))

        # 3. BEV 融合
        if self.fusion_mode == "concat":
            # 将合并的 B*V 变回 (B, V * feat_ch, Hb, Wb)
            bev_fused = bev_flat.reshape(B, V * bev_flat.size(1), self.Hb, self.Wb)
        else:
            # 变回 (B, V, feat_ch, Hb, Wb) 并应用置信度融合
            bev_stack = bev_flat.reshape(B, V, bev_flat.size(1), self.Hb, self.Wb)
