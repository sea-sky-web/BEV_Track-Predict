...
理由：审计发现当前所有头条数字均存在协议或代码层面的致命缺陷，修复后数字会变化（方向已知，幅度未知），后续所有消融和写作必须基于修复后的数字。

### P0 修复顺序（必须全部完成才能进入写作）

| 顺序 | 项目 | 修复内容 | 预估影响 |
|:---:|------|------|------|
| 1 | D2 | 从真实 tracker 输出重新生成 fig9，或删除该图 | 消除学术诚信风险 |
| 2 | A6 | 加 `torch.manual_seed` + 验证集（320–359） | 使 B1 修复成为可能 |
| 3 | A3 | 从 `annotations_positions/*.json` 取世界坐标 GT | MODA 可能下降（n_gt 恢复） |
| 4 | A4 | 统一为贪心匹配 + 0.5 m | MODA 可能下降（Hungarian→贪心） |
| 5 | B1 | 固定超参（threshold=0.400, NMS=6.0），在验证集上选，测试集只跑一次 | MODA 会下降 |
| 6 | C1 | 常速度末点改用后向差分 | 常速度 ADE/FDE 上升，MLP 可能胜出 |
| 7 | C3 | AUPRC 统一阈值网格 | ConvLSTM 原分辨率 AUPRC 可能大幅上升 |
